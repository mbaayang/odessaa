	  ...
	  if (confirmed && selectedOption) {
		if (selectedOption === 'confirm') {
		  if (clickedOrder.state !== 'sale') {
			const result = await this.orm.call('sale.order', 'action_confirm', [clickedOrder.id]);

			// Trigger a refresh of the order list after confirmation
			this.saleOrderFetcher.fetch(); // Refresh orders

			// Optional: Display confirmation message (can be customized)
			await this.popup.add(ConfirmPopup, {
			  title: _t('Confirmed!'),
			  body: _t('Sale Order ') + clickedOrder.name + ' ' + _t('is confirmed.'),
			});
		  } else {
			await this.popup.add(ConfirmPopup, {
		  ...
		  if (clickedOrder.state !== 'cancel') {
			const result = await this.orm.call('sale.order', 'action_cancel', [clickedOrder.id]);

			// Trigger a refresh of the order list after cancellation
			this.saleOrderFetcher.fetch(); // Refresh orders

			// Optional: Display confirmation message (can be customized)
			await this.popup.add(ConfirmPopup, {
			  title: _t('Cancelled!'),
			  body: _t('Sale Order ') + clickedOrder.name + ' ' + _t('is cancelled.'),
			});
		  } else {
			await this.popup.add(ConfirmPopup, {
